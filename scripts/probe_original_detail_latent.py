"""Single-factor original-detail loss comparison on three independent sample latents.
...
from scripts.plot_original_detail_latent_losses import plot_run
from scripts.probe_latent_reachability import check as check_control
...
from scripts.verify_loss_balance_initialization import assert_equal
...
CONTROL = ROOT / "logs/latent-reachability-20260909-v1"
DETAIL_WEIGHT = .5
INIT_FIELDS = ("model", "optimizer", "scheduler", "scaler", "sampler", "rng")
...
def codes():
    result = code_hashes()
    for name in ("probe_latent_reachability.py", "probe_original_detail_latent.py", "plot_original_detail_latent_losses.py", "diagnose_supervision_gradients.py", "detail_supervision_math.py"):
...
def declare(output):
    ...
    require(summary["status"] == "verified_completed_target_assisted_sample_latent_diagnostic"
            and summary["source_optimizer_updates"] == 96 and summary["new_optimizer_updates"] == 0
            and summary["inputs"][0] == bound(CONTROL / "protocol.json"), "Verified completed control required")
    config = load_config(control["config"])
    with no_training_guard(), TrainingBudget(ROOT / "runs", config.project.budget_seconds, phase="original_detail_latent_declare") as budget:
        ...
        protocol = {"status": "declared_before_optimizer_updates", "kind": KIND,
            ...
            "authorization": "User explicitly continued research after the proposed original-detail single-factor latent comparison",
            ...
            "question": "Does adding original-target detail supervision improve the same bounded latent optimization? No AI or R change.",
    ...
    print(canonical_json({"event": "original_detail_latent_declared", "output": str(output), "maximum_optimizer_updates": 96}).decode(), flush=True)
...
def assert_same_initialization(control, current):
    ...
    require(set(control["model"]) == set(current["model"]) == {"delta_latent"}
            and torch.count_nonzero(current["model"]["delta_latent"]).item() == 0,
            "Initial state must contain one zero sample delta")
...
def check_preflight(output, p):
    ...
    result = read(path)
    require(result["status"] == "completed_original_detail_latent_preflight"
    ...
    for saved, case in zip(result["cases"], p["cases"]):
        require(saved["case"] == case["name"] and saved["view_id"] == case["view_id"]
                ...
                and saved["delta_unchanged"] and saved["identity_output"] and saved["param_grad_none"],
...
def preflight(output):
    ...
    with TrainingBudget(ROOT / "runs", config.project.budget_seconds, phase="original_detail_latent_preflight") as budget, gradient_only_guard() as guard:
        ...
        try:
            ...
            for case in p["cases"]:
                ...
                require(torch.equal(prediction,sample["x"]), "Zero delta must produce exact input")
                ...
                require(container.delta_latent.grad is None and torch.count_nonzero(container.delta_latent)==0,
                ...
                rows.append({"case":case["name"],"view_id":case["view_id"],"original_index":case["index"],"source":case["source"],
                    ...
                    "identity_output":True,"delta_unchanged":True,"param_grad_none":True})
            ...
            check(p)
            result={"status":"completed_original_detail_latent_preflight","protocol":bound(output/"protocol.json"),"cases":rows,
    ...
    print(canonical_json({"event":"original_detail_latent_preflight_completed","autograd_grad_calls":9,"optimizer_updates":0}).decode(),flush=True)
...
def train_case(output, case_name, resume=False):
    ...
    with TrainingBudget(ROOT / "runs",config.project.budget_seconds,phase="original_detail_latent_"+case_name) as budget:
        try:
            ...
            if checkpoint is None:
                ...
                atomic_write(run/"same_initialization.json",canonical_json({"control_checkpoint":bound(control_path),
                    "current_checkpoint":bound(checkpoint),"equal_fields":list(INIT_FIELDS),"all_equal":True}))
                del control_state
            else:
                control_case=next(c for c in p["control_cases"] if c["name"]==case_name)
                check_initialization_record(run,Path(control_case["run"]),budget)
            torch.cuda.reset_peak_memory_stats()
            ...
            while step<STEPS:
                ...
                print(canonical_json({"event":"original_detail_latent_update","case":case_name,"step":step,
            ...
            report={"status":"completed_original_detail_sample_latent_probe","kind":KIND,"phase":"pixel",
