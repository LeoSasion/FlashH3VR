...
OUT=ROOT/'logs/h3ce-native-video-benchmark-20260909-v1'
...
def main():
    ...
    files=[Path(__file__),ROOT/'scripts/video_benchmark_math.py',ROOT/'components.lock.json',REFERENCE,
           PARENT/'training_contract.json',Path(checkpoint['path']),SOURCE]
    protocol={'kind':'one_pass_current_spatial_delta_native_h3_video','authorization':'User selected fastest VOSR2 record and authorized our algorithm; subsequently limited our algorithm to ONE full run',
      ...
      'spatial_precision':'FP32 no autocast, existing tile256/overlap32; per-frame2D; no learned temporal branch',
      ...
      'maximum_spatial_tiles':400,'maximum_h3_encodes':12,'maximum_h3_decodes':12,
      ...
      'spatial_tile_grid':tile_grid(112,192),'gpu_idle_gate':'Ten consecutive one-second samples <=10% before load and before inference; max180sec per gate; no extra forward',
      'quality_acceptance':False,'product_tier_acceptance':False}
    save(OUT/'protocol.json',protocol)
    counts={'spatial_tiles':0,'encodes':0,'decodes':0,'chunks':0};stages={};telemetry=Telemetry();handles=[]
    try:
      with TrainingBudget(ROOT/'runs',172800,phase='one_h3ce_native_video_benchmark') as budget,no_training_guard() as guard:
        ...
        def hook(m,args):
            counts['spatial_tiles']+=1
            assert counts['spatial_tiles']<=400
        ...
        for i in range(100):
            quarter=F.interpolate(x[:,:,i],scale_factor=.25,mode='area')
            b=tiled_restore(base,quarter)[1];c=tiled_restore(candidate,quarter)[1]
            spatial[:,:,i]=(x[:,:,i]+(c-b)).clamp(0,1)
            del quarter,b,c
            if (i+1)%20==0:print(f'Spatial frames {i+1}/100; elapsed {time.perf_counter()-start:.2f}s',flush=True)
        ...
        assert counts=={'spatial_tiles':400,'encodes':12,'decodes':12,'chunks':6}
