"""Observe C4 original-detail sample-latent records, without starting any work.
...
def bound_evidence(run):
    ...
    _require(isinstance(experiment, dict) and experiment.get("kind") == KIND
             and contract.get("phase") == "pixel", "Run is not a C4 original-detail latent diagnostic")
    _require(experiment.get("refiner_loaded") is False and experiment.get("deployable") is False,
             "C4 must not load a refiner or claim deployable output")
    ...
    return {
        ...
        "wrapper_source_sha256": hashes["scripts/plot_target_start_latent_losses.py"],
...
def _plot_context(bindings, window):
    ...
    with _PLOT_LOCK:
        ...
        def savefig(figure, *args, **kwargs):
            ...
            figure.suptitle("C4 target-start per-sample latent optimization | H3 frozen; R not loaded", fontsize=11)
...
def plot_runs(runs, output, window=4, live=False):
    ...
    _require(runs and len(runs) <= 3 and len(set(runs)) == len(runs), "Select one to three distinct C4 case runs")
