"""Run one image or one real 22-frame video head tensor through FlashH3VR."""
...
def _read_video_window(path: Path) -> torch.Tensor:
    ...
    if (array.dtype != np.float32 or array.ndim != 4 or array.shape[0] != 22
            or array.shape[-1] != 3 or array.shape[1] != array.shape[2]
            or not np.isfinite(array).all() or np.any((array < 0) | (array > 1))):
        raise ValueError("Video NPY must be float32 RGB [22,S,S,3] in [0,1]")
    return torch.from_numpy(np.ascontiguousarray(array.transpose(3, 0, 1, 2)))[None]


def main() -> None:
    parser = argparse.ArgumentParser(description="Frozen H3 + adopted Dense head-window restoration")
    ...
    parser.add_argument("--input", type=Path, required=True,
                        help="RGB PNG for image or float32 [22,S,S,3] NPY for video")
    parser.add_argument("--output", type=Path, required=True,
                        help="RGB PNG for image or float32 [22,S,S,3] NPY for video")
    parser.add_argument("--pts-json", type=Path,
                        help="Required for video: JSON array of 22 real increasing PTS seconds")
    ...
    parser.add_argument("--target-side", type=int, choices=BUCKETS,
                        help="Required with --half-input; one of 256/448/640/832")
    ...
    if args.kind == "video":
        ...
        try:
            FrameMeta("video", tuple(float(value) for value in pts), True).validate(22)
