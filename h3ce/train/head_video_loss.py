"""Target frame-difference supervision after per-frame original-coordinate paste.
...
def target_transition_loss(prediction, target, support, pts, shot_ids, *,
                           valid_frames=None, max_gap_seconds=.1, epsilon=.001):
    ...
    if (not math.isfinite(max_gap_seconds) or max_gap_seconds <= 0
        ...
        raise ValueError('Positive finite gap and Charbonnier epsilon required')
    ...
    # Subtract the constant epsilon so an exact target sequence has zero loss.
    per_pair=((torch.sqrt(error.square()+epsilon**2)-epsilon)*mask).flatten(1).sum(1)/(3*area)
    return {'total':per_pair.mean(),'per_transition':per_pair,'pair_indices':[(i-1,i) for i in indices],
            'support_area':area,'eligible_transitions':len(indices),'pts_units':'seconds; per-transition difference'}
